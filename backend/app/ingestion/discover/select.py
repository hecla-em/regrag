...
def filter_acts_by_basis_article(
    acts: list[CandidateAct], base_celex: str, prefixes: tuple[str, ...]
) -> list[CandidateAct]:
    """The base act, and the acts adopted under an article starting with one of the prefixes."""
    return [
        act
        for act in acts
        if act.celex == base_celex or any(a.startswith(prefixes) for a in act.basis_articles)
    ]
