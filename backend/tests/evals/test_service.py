...
async def test_a_run_retrieves_unless_told_not_to(answering_graph: None) -> None:
...
async def test_a_stored_run_keeps_that_it_ran_without_retrieval(db_session) -> None:
    created = await create_eval_run(db_session, eval_run(eval_result(), retrieval=False))
    db_session.expunge_all()

    assert (await get_eval_run(db_session, created.id)).retrieval is False
